...
GROUP_TITLE = "داشبورد test16 — كل المجموعات"
# test16
...
TONIGHT_DB  = os.path.join(DATA_DIR, "tonight_watchlist_test16_unified.db")
SIGNALS_DB  = os.path.join(DATA_DIR, "signals_test16_unified.db")
...
for sym, name in ALL_STOCKS:
    if sym not in seen_s:
        seen_s.add(sym)
        ALL_STOCKS_FLAT.append((sym, name))
# ============================================================

# ============================================================
# 2. قوائم الأسهم — 4 مجموعات
# ============================================================
...
GROUPS = {
    "1️⃣ كبار السوق": [
        ...
        ("1202","مبكو"),("2081","شركة الخريف لتقنية المياه والطاقة"),
        ("2060","شركة التصنيع الوطنية"),("2382","شركة أديس القابضة"),
        ("1182","شركة أملاك العالمية للتمويل"),("1183","شركة سهل للتمويل"),
        ("4013","مجموعة الدكتور سليمان الحبيب"),
        ("4009","شركة الشرق الأوسط للرعاية الصحية"),("4164","شركة النهدي الطبية"),
        ("4190","شركة جرير للتسويق"),("4240","سينومي ريتيل"),
        ("4001","شركة أسواق عبدالله العثيم"),
        ("4300","شركة دار الأركان للتطوير العقاري"),
        ("4230","شركة البحر الأحمر العالمية"),("4220","إعمار المدينة الإقتصادية"),
        ("2280","شركة المراعي"),("2050","مجموعة صافولا"),
    ],
    "2️⃣ الصناعات والأسمنت": [
        ("1201","شركة تكوين المتطورة للصناعات"),
        ("1210","شركة الصناعات الكيميائية الأساسية"),
        ("1213","شركة نسيج العالمية التجارية"),("1214","شركة الحسن غازي إبراهيم شاكر"),
    ...
    "3️⃣ التجزئة والغذاء": [
        ...
        ("4280","شركة المملكة القابضة"),("4320","شركة الأندلس العقارية"),
        ("2270","الشركة السعودية لمنتجات الألبان والأغذية"),
        ("2281","شركة التنمية الغذائية"),("2282","شركة نقي للمياه"),
        ...
        ("6004","شركة كاتريون للتموين"),("4080","شركة سناد القابضة"),
    ],
    "4️⃣ الريت والخدمات": [
        ...
        ("4292","شركة عطاء التعليمية"),("6019","شركة المسار الشامل للتعليم"),
        ...
        ("4180","مجموعة فتيحي القابضة"),("6020","شركة القصيم القابضة"),
    ],
}

# كل الأسهم مجمعة بدون تكرار
ALL_STOCKS_FLAT = []
seen_all = set()
for grp_stocks in GROUPS.values():
    for sym, name in grp_stocks:
        if sym not in seen_all:
            seen_all.add(sym)
            ALL_STOCKS_FLAT.append((sym, name))
...
st.set_page_config(
    page_title="داشبورد — test16 الموحد",
    layout="wide",
    page_icon="📊",
...
def get_all_quotes():
    all_syms = list({s for s, _ in ALL_STOCKS_FLAT})
    quotes_dict, errors = {}, []
...
all_data, failed_syms = analyze_stocks(
    ALL_STOCKS_FLAT, quotes_dict, tasi_change, tasi_up, tasi_down, now_mins, tonight_syms
)
...
if after_close and all_data:
    scan_tonight(ALL_STOCKS_FLAT, quotes_dict)
...
# ============================================================
# التابات الموحدة — 4 مجموعات + تقارير + backtest
# ============================================================

# تاب لكل مجموعة + تقارير مشتركة
grp_names = list(GROUPS.keys())
tab_g1, tab_g2, tab_g3, tab_g4, tab_reports_u, tab_backtest_u = st.tabs([
    grp_names[0],
    grp_names[1],
    grp_names[2],
    grp_names[3],
...
def render_group_tab(grp_name, grp_stocks):
    """يعرض محتوى تاب مجموعة كاملة"""

    # قائمة فريدة
    seen_g = set()
    unique_grp = []
    for sym, name in grp_stocks:
        if sym not in seen_g:
            seen_g.add(sym)
            unique_grp.append((sym, name))

    st.caption(f"📋 {grp_name} — {len(unique_grp)} سهم")

    # جلب الكوتس لهذه المجموعة فقط
    grp_syms = [s for s, _ in unique_grp]
    grp_quotes = {s: quotes_dict[s] for s in grp_syms if s in quotes_dict}

    # تحليل أسهم المجموعة فقط
    with st.spinner(f"جاري مسح {grp_name}..."):
        grp_data, grp_failed = analyze_stocks(
            unique_grp, grp_quotes,
            tasi_change, tasi_up, tasi_down,
            now_mins, tonight_syms
        )

    if not grp_data:
        st.warning("لا توجد بيانات — تحقق من الاتصال")
        return

    df_grp = pd.DataFrame(grp_data)
    df_buy_grp = df_grp[df_grp["الإشارة"] == "BUY 🟢"].sort_values("الثقة%", ascending=False)
    buy_n = len(df_buy_grp)

    # ملخص سريع
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("إجمالي الأسهم", len(grp_data))
    c2.metric("BUY 🟢", buy_n)
    c3.metric("فشل", len(grp_failed))
    c4.metric("آخر مسح", st.session_state.last_scan_time or "—")

    # أفضل فرصة
    if not df_buy_grp.empty:
        st.success(f"⚡ أفضل فرصة: **{df_buy_grp.iloc[0]['الاسم']}** ({df_buy_grp.iloc[0]['الرمز']}) — ثقة {df_buy_grp.iloc[0]['الثقة%']}%")

    # الجدول
    show_c = ["الرمز","الاسم","السعر","التغيير%","النجوم","الإشارة",
              "هدف1","هدف2","Stop Loss","الثقة%","RSI","تحذير RSI",
              "MACD زخم","حجم×","صافي السيولة (M)",
              "توصية المحللين","هامش الأمان%",
              "القوة%","Breakout","في قائمة الغد","سبب WAIT"]
    show_c = [c for c in show_c if c in df_grp.columns]

    # فلاتر
    f1, f2, f3 = st.columns(3)
    with f1: only_buy = st.checkbox("BUY فقط", key=f"buy_{grp_name}")
    with f2: hide_rsi = st.checkbox("إخفاء ذروة RSI", key=f"rsi_{grp_name}")
    with f3: sort_by  = st.selectbox("ترتيب:", ["الثقة%","القوة%","RSI","حجم×"], key=f"sort_{grp_name}")

    df_show = df_buy_grp.copy() if only_buy else df_grp.copy()
    if hide_rsi and "تحذير RSI" in df_show.columns:
        df_show = df_show[~df_show["تحذير RSI"].str.contains("🔴", na=False)]
    df_show = df_show.sort_values(sort_by, ascending=(sort_by == "RSI"))

    st.dataframe(df_show[show_c], use_container_width=True, height=450,
        column_config={
    ...
    csv = df_show[show_c].to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
    st.download_button(f"⬇️ تحميل {grp_name}", csv,
                       f"grp_{grp_name[:3]}_{now.strftime('%Y_%m_%d')}.csv", "text/csv",
                       key=f"dl_{grp_name}")


# ── عرض كل تاب عند الضغط عليه فقط ──
with tab_g1:
    render_group_tab(grp_names[0], GROUPS[grp_names[0]])

with tab_g2:
    render_group_tab(grp_names[1], GROUPS[grp_names[1]])

with tab_g3:
    render_group_tab(grp_names[2], GROUPS[grp_names[2]])

with tab_g4:
    render_group_tab(grp_names[3], GROUPS[grp_names[3]])
    ...
    **كيف يعمل:**
    ...
    if run_bt:
        ...
        if bt_scope == "أسهم سريعة (50 سهم)":
            stocks_to_test = random.sample(ALL_STOCKS_FLAT, min(50, len(ALL_STOCKS_FLAT)))
        else:
            stocks_to_test = ALL_STOCKS_FLAT
